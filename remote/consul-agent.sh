...
/usr/local/bin/consul agent -bootstrap-expect 3 -server -ui-dir  /usr/local/consul_ui -encrypt $(cat /var/easydeploy/share/.config/serf_key) -config-dir /etc/consul.d -data-dir /var/easydeploy/.consul_state -node ${MACHINE_NAME}-${EASYDEPLOY_HOST_IP}  -advertise ${EASYDEPLOY_HOST_IP} -bind=${EASYDEPLOY_HOST_IP} -rejoin ${client_flag} || (sleep 20 && exit -1)