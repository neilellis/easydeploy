...
/usr/local/bin/serf agent -snapshot /var/serf/snapshot -encrypt=$(cat /var/easydeploy/share/.config/serf_key) -node=${DEPLOY_ENV}-${PROJECT}-${COMPONENT}-${EASYDEPLOY_HOST_IP} -bind ${EASYDEPLOY_HOST_IP} -tag deploy_env=${DEPLOY_ENV} -tag project=${PROJECT} -tag component=${COMPONENT} -event-handler /etc/serf/event-handler.sh  -event-handler  member-join,member-leave,member-failed,member-update,member-reap=/etc/serf/handlers/membership-handler.sh || (sleep 20 && exit -1)