...
dockerImage=$(< /var/easydeploy/share/.config/hostname)
...
export CONTAINER=$(sudo weave run ${weave_net} --name ${COMPONENT}-${1} -t -i --sig-proxy $DOCKER_ARGS -v /home/easydeploy/usr/etc/container:/var/easydeploy/etc -v /var/easydeploy/container/$1:/var/local -v /var/log/easydeploy/container/$1:/var/log/easydeploy -v /var/easydeploy/container/$1/data:/data -v /var/easydeploy/share:/var/share -v /var/easydeploy/share:/var/easydeploy/share -e EASYDEPLOY_HOST_IP=${EASYDEPLOY_HOST_IP} -e WEAVE_IP=${weave_ip} -e WEAVE_SUBNET=${weave_subnet} --dns ${EASYDEPLOY_HOST_IP} ${dockerLinks} ${dockerImage} ${DOCKER_COMMANDS})
