...
then
   ...
   ../providers/${PROVIDER}/list-machines-by-ip.sh $(mc_name) | parallel --gnu -P 0  "set -eux; sync ${DIR}/  easydeploy@{}:~/project/;  ssh  -o \"StrictHostKeyChecking no\" easydeploy@${machine} \" cd ~/project/; docker build .\" ssh  -o 'StrictHostKeyChecking no' easyadmin@{} 'sudo /usr/bin/supervisorctl restart \$(cat /var/easydeploy/share/.config/component):' "
fi
